...
import {
  type AdminProdutoListQuery,
  type CreateProdutoInput,
  type ProdutoParams,
  type UpdateProdutoInput,
  adminProdutoListQuerySchema,
  createProdutoSchema,
  produtoParamsSchema,
  updateProdutoSchema,
...
const listQuerySchema = adminProdutoListQuerySchema;
...
export async function adminProductRoutes(app: FastifyInstance): Promise<void> {
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      const query = request.query as AdminProdutoListQuery;
      ...
      const where: Record<string, unknown> = { loja_id: lojaId, status: { not: 'ARQUIVADO' } };
  ...
  app.post(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const existingSku = await prisma.produto.findFirst({
        where: { loja_id: lojaId, sku: body.sku },
      });
      ...
      if (body.codigo_barras) {
        const existingBarras = await prisma.produto.findFirst({
          where: { loja_id: lojaId, codigo_barras: body.codigo_barras },
        });
        if (existingBarras) {
      ...
      const existingSlug = await prisma.produto.findFirst({
        where: { loja_id: lojaId, slug: body.slug },
      });
      ...
      if (!categoria || categoria.loja_id !== lojaId) {
  ...
  app.put(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const existing = await prisma.produto.findFirst({ where: { id, loja_id: lojaId } });
      if (!existing) {
        return reply.code(404).send({ error: 'Produto não encontrado' });
      }
  ...
  app.delete(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const produto = await prisma.produto.findFirst({ where: { id, loja_id: lojaId } });
      if (!produto) {
        return reply.code(404).send({ error: 'Produto não encontrado' });
      }
